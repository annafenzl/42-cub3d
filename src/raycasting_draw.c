...
void draw(t_cub *cub, t_ray *ray, int x)
{
	...
	tex_pos = (ray->draw_start - cub->mlx->height / 2 + ray->line_height / 2) * step;
	y = ray->draw_start;
	while (y <= ray->draw_end)
	{
		tex_y = (int)tex_pos & (TEXTURE_SIZE - 1);
		tex_pos += step;
		ft_memcpy(&cub->window.window_img->
			pixels[(y * (int)cub->mlx->width + x) * BPP],
			&cub->tex_dir[get_texture_index(ray)]->
			pixels[(tex_y * cub->tex_dir[0]->width + tex_x) * BPP], BPP);
		y++;
	}
}